...
func (h *DocumentHandler) handlePostDocumentLabel(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.UpdateDocument(ctx, d, influxdb.Authorized(a)); err != nil {
		EncodeError(ctx, err, w)
		return
	}
...
func (h *DocumentHandler) handleDeleteDocumentLabel(w http.ResponseWriter, r *http.Request) {
	...
	for k, l := range d.Labels {
		if req.LabelID == l.ID {
			d.Labels = append(d.Labels[:k], d.Labels[k+1:]...)
			break
		}
	}
...
func (h *DocumentHandler) getDocument(w http.ResponseWriter, r *http.Request) (*influxdb.Document, string, influxdb.DocumentStore, influxdb.Authorizer, error) {
	ctx := r.Context()
...
func (h *DocumentHandler) handleGetDocument(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	d, namspace, _, _, err := h.getDocument(w, r)
	if err != nil {
		EncodeError(ctx, err, w)
		return
	}
